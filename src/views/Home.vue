<template>
  <div class="home">
    <div v-if="!currentEvent && !nextEvent" class="ma-16 text-center">
      <v-progress-circular
        :size="100"
        :width="10"
        color="#37003c"
    ...
    <v-container class="mt-8 mb-16" v-if="currentEvent && currentEvent.length">
      ...
      <div class="ma-8" v-for="i in currentEvent" :key="i.name">
        <v-card class="pa-4 text-center">
          ...
          <div>Highest score: {{ i.highest_score || "-" }}</div>
          <div>Most captained: {{ i.most_captained || "-" }}</div>
          <div>Most selected: {{ i.most_selected || "-" }}</div>
          <div>Most vice_captained: {{ i.most_vice_captained || "-" }}</div>
          <div>Top element: {{ i.top_element || "-" }}</div>
        </v-card>
      </div>
    </v-container>
    <v-container class="mt-8 mb-16" v-if="nextEvent && nextEvent.length">
    ...
    <v-container class="mt-8 mb-16" v-if="lastEvent && lastEvent.length">
